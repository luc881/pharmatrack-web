...
export function OdDivulgacion({ articles = [] }) {
  ...
  return (
    ...
    >
      <Display
        size="clamp(48px, 11.4vw, 176px)"
        weight={400}
        sx={{ lineHeight: 1, letterSpacing: '-0.015em', textTransform: 'uppercase', mb: 1.5 }}
      >
        Divulgación
      </Display>
      <Box
      ...
      >
        {current.map((article, i) => (
          <Box
            key={article.id}
            className="od-wipe"
            sx={{ animation: `odWipe 1000ms var(--od-ease) ${i * 0.38}s both` }}
          >
            <Link
              ...
              sx={{
                ...
                gridTemplateColumns: {
                  xs: '1fr',
                  md: 'minmax(90px, 0.5fr) minmax(240px, 1.5fr) minmax(280px, 2fr)',
                },
            ...
            >
              ...
              <Box sx={{ pb: { md: '34px' } }}>
                ...
                {article.reading_minutes != null && (
                  <Box sx={{ mt: 2.5, fontSize: 13, color: 'var(--color-neutral-600)' }}>
