  ...
  return (
    ...
    >
      ...
      <Box sx={{ minWidth: 0, flexGrow: 1 }}>
        ...
        <Box sx={{ mt: 0.75, fontSize: 15, fontVariantNumeric: 'tabular-nums' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
        ...
        <Box
          ...
          sx={{
            ...circleBtn,
            ...(atMax && { cursor: 'default', color: 'var(--color-neutral-400)', '&:hover': {} }),
          }}
        >
          +
        </Box>
    ...
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', py: 1 }}>
      <Box
        sx={{
          ...
          color: strong ? 'var(--color-text)' : 'var(--color-neutral-700)',
        }}
      >
        {label}
      </Box>
      <Box sx={{ fontSize: 15, fontVariantNumeric: 'tabular-nums', color: 'var(--color-text)' }}>
        {value}
      </Box>
    </Box>
  );
      ...
      {STEPS.map((label, i) => {
        ...
        return (
          <Box key={label} sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 2 } }}>
            ...
            {i < STEPS.length - 1 && (
              <Box
                sx={{ width: { xs: 18, sm: 40 }, height: '1px', bgcolor: 'var(--color-divider)' }}
              />
            )}
          </Box>
        );
...
export function OdCartView() {
  ...
  useEffect(() => {
    ...
    fetch('/api/cart-validate', {
      ...
      .then((data) => {
        ...
        const msgs = [];
        if (removed.length)
          msgs.push(`Quitamos lo que ya no está disponible: ${removed.join(', ')}.`);
        if (adjusted.length)
          msgs.push(`Ajustamos la cantidad a lo disponible: ${adjusted.join(', ')}.`);
        setNotice(msgs.join(' '));
      })
      .catch(() => {});
  }, [items, replaceAll]);
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Box
            component="span"
            sx={{ fontFamily: 'var(--font-heading)', fontSize: 22, color: 'var(--color-text)' }}
          >
            {confirmed.code}
          </Box>
          . Te escribimos por WhatsApp para coordinar la entrega. También te llegó un correo con el
          detalle.
        </Box>
  ...
  return (
    <Box
      component="section"
      sx={{ px: { xs: '18px', md: '40px' }, pt: { xs: 3, md: 5 }, pb: { xs: 8, md: 12 } }}
    >
      <Display
        component="h1"
        size="clamp(48px, 7vw, 104px)"
        sx={{ textAlign: 'center', lineHeight: 1, mb: { xs: 4, md: 6 } }}
      >
        ...
        <>
          ...
          <Box
            sx={{
              ...
              gap: { xs: 4, md: '48px' },
              alignItems: 'start',
              gridTemplateColumns: { xs: '1fr', md: 'minmax(0, 1.5fr) minmax(340px, 0.62fr)' },
            }}
          >
                ...
                {items.map((item) =>
                  step === 0 ? (
                    ...
                    >
                      ...
                      <Box sx={{ minWidth: 0, flexGrow: 1 }}>
                        <Box sx={{ fontFamily: 'var(--font-heading)', fontSize: 16 }}>
                          {item.title}
                        </Box>
            ...
            >
              ...
              <SummaryRow
                label={isPickup ? 'Entrega en CDMX' : 'Envío'}
                value={isPickup ? fCurrency(0) : 'Por cotizar'}
              />
              ...
              <SummaryRow
                label="Total"
                value={
                  <Box component="span" sx={{ fontFamily: 'var(--font-heading)', fontSize: 26 }}>
                    {fCurrency(total)}
                  </Box>
                }
                strong
                ...
                <>
                  {!shippingEnabled && (
                    <Alert
                      severity="info"
                      icon={false}
                      sx={{ mt: 2, py: 0.5, typography: 'caption' }}
                    >
                  ...
                  {signedIn ? (
                    <>
                      ...
                      <TextField
                        ...
                        helperText={
                          isPickup
                            ? 'Para coordinar la entrega. Queda guardada en tu cuenta.'
                    ...
                    <>
                      <Box
                        sx={{ fontSize: 14, color: 'var(--color-neutral-700)', lineHeight: 1.6 }}
                      >
                ...
                <Box sx={{ mt: 2.5 }}>
                  ...
                  >
                    <Box>
                      <strong style={{ color: 'var(--color-text)' }}>
                        {isPickup ? 'Entrega en persona · CDMX' : 'Envío a domicilio'}
                      </strong>
                    </Box>
                    <Box>
                      {isPickup
                        ? 'Punto y hora se acuerdan por WhatsApp. Sin costo.'
                        : 'Coordinamos costo y guía por WhatsApp.'}
                    </Box>
                  ...
                  <Box
                    ...
                    onSubmit={(e) => {
                      e.preventDefault();
                      setPromoMsg(
                        promo.trim() ? 'Por ahora no hay códigos de promoción activos.' : ''
                      );
                    }}
                    sx={{
                      display: 'flex',
                  ...
                  {promoMsg && (
                    <Box sx={{ mt: 1, fontSize: 12, color: 'var(--color-neutral-600)' }}>
                      {promoMsg}
                    </Box>
                  )}
