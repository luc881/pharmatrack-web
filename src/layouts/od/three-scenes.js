...
export function buildJar(THREE, { scene }) {
  ...
  const tray = new THREE.Mesh(
    new THREE.CylinderGeometry(R + 0.12, R + 0.12, 0.55, 72),
    new THREE.MeshStandardMaterial({ color: 0x1b1917, roughness: 0.7, metalness: 0.1 })
  );
  tray.position.y = -H / 2 + 0.18;
  g.add(tray);
  const water = new THREE.Mesh(
    new THREE.CircleGeometry(R - 0.06, 72),
    new THREE.MeshStandardMaterial({ color: 0x2b2a24, roughness: 0.25, metalness: 0.2 })
  );
  ...
  const soil = new THREE.Mesh(
    new THREE.CylinderGeometry(R - 0.08, R - 0.08, 0.5, 64),
  ...
  for (let i = 0; i < 22; i += 1) {
    ...
    t.position.set(
      Math.cos(a) * r,
      -H / 2 + 1.05 + Math.random() * 0.55 - r * 0.35,
      Math.sin(a) * r
    );
    t.scale.y = 0.7;
    g.add(t);
  ...
  for (let i = 0; i < 3; i += 1) {
    const b = new THREE.Mesh(
      new THREE.CylinderGeometry(0.1, 0.16, 1.4 + Math.random() * 0.7, 7),
      barkMat
    );
    const a = i * 2.1 + 0.6;
  ...
  [-1, 1].forEach((sgn) => {
    ...
    for (let k = 1; k <= 3; k += 1) {
      const rg = new THREE.Mesh(
        new THREE.TorusGeometry(0.19 * k, 0.012, 6, 30),
...
export function buildStack(THREE, { scene }) {
  ...
  g.add(tableTop);
  const tray = new THREE.Mesh(
    new THREE.CylinderGeometry(2.0, 2.0, 0.5, 48),
    new THREE.MeshStandardMaterial({ color: 0xcfcdc8, roughness: 0.6 })
  );
  ...
  rows.forEach((count, ri) => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const jar = new THREE.Mesh(
        new THREE.CylinderGeometry(0.34, 0.34, 0.72, 28),
        new THREE.MeshStandardMaterial({
          color: palette[(ri * 3 + i) % palette.length],
          roughness: 0.42,
        })
      );
...
export const SCENES = {
  ...
  stack: {
    build: buildStack,
    opts: { fov: 30, position: [0, 3.4, 17], lookAt: [0, -0.6, 0], background: 0xd8d9d6 },
  },
};
