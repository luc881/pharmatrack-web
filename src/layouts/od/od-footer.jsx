...
export function OdFooter() {
  const now = useClock();
  const clock = now
    ? now.toLocaleTimeString('es-MX', { hour: 'numeric', minute: '2-digit', second: '2-digit' })
    : '';
  const today = now
    ...
    >
      <Box
        sx={{
          ...
          gridTemplateColumns: {
            xs: '1fr',
            md: 'minmax(320px, 1.5fr) minmax(240px, 1fr) minmax(160px, 0.6fr)',
          },
        }}
      >
        {/* Navegación */}
        ...
        <Box>
          ...
          >
            <Box
              component="span"
              sx={{ width: 7, height: 7, borderRadius: '50%', bgcolor: 'var(--color-accent-400)' }}
            />
            Apartados abiertos
          </Box>
          <Box sx={{ mt: 3, mb: 2.25 }}>
            ...
            {REDES.map((r) => (
              <Link
                ...
                {...(r.external
                  ...
                  : { component: RouterLink, href: r.href })}
                sx={{
                  color: 'var(--color-neutral-100)',
                  textDecoration: 'none',
                  transition: 'color 300ms',
          ...
          <Box>
            <Link
              ...
              sx={{
                color: 'inherit',
                textDecoration: 'none',
                '&:hover': { color: 'var(--color-neutral-200)' },
              }}
            >
              Volver arriba ↑
            </Link>
